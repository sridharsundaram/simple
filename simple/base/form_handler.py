...
class FormHandler(webapp.RequestHandler):
  ...
  def get_value(self, form, field, type):
    ...
    if type == 'StringProperty':
      return value
    elif type == 'DateProperty':
      return None if value == 'None' else date(int(value[0:4]), int(value[5:7]), int(value[8:10]))
    elif type == 'BooleanProperty':
      return value == 'on'
    elif type == 'IntegerProperty':
      return 0 if value == 'None' else int(value)
    elif type == 'PhoneNumberProperty':
      return db.PhoneNumber(value)
